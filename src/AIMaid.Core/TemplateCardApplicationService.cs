    ...
    ICommandHandler<GenerateTemplateCardCommand, OperationResult<CharacterDto>>
{
    ...
    private async Task<LlmSourcePromptDto> LoadSourcePromptAsync(CancellationToken cancellationToken)
    {
        ...
        if (json is not null)
        {
            ...
            if (saved is not null && saved.IsEnabled) return saved;
            throw new InvalidOperationException("角色卡生成 Source Prompt 已停用或无效。");
        }
        var now = DateTimeOffset.Now;
        var seeded = new LlmSourcePromptDto(
            ...
            true, now, now);
        await documents.UpsertAsync(SourcePromptDomain, seeded.SourceKey, JsonSerializer.Serialize(seeded), now, cancellationToken);
        return seeded;
    }
