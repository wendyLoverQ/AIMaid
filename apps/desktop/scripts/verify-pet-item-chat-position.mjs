...
try {
  ...
  await delay(500)
  const itemBounds = await evaluate(pet, `(() => {
    const rect = document.querySelector('.ui-pet-item').getBoundingClientRect();
    return { x: screenX + rect.x, y: screenY + rect.y, width: rect.width, height: rect.height };
  })()`)
  ...
  const chatBounds = await evaluate(chat, `({ x: screenX, y: screenY, width: outerWidth, height: outerHeight })`)
  ...
  const proof = { itemBounds, itemCenter, chatBounds, chatCenter, delta }
