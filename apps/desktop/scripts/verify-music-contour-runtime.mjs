...
try {
  ...
  const metrics = await evaluate(client, `(() => {
    ...
    return {
      sameTransformParent: source.parentElement === overlay.parentElement,
  ...
  const aligned = ['x', 'y', 'width', 'height'].every((key) =>
    Math.abs(metrics.sourceBounds[key] - metrics.overlayBounds[key]) <= 0.5)
  if (!metrics.sameTransformParent || !aligned || metrics.visiblePixels <= 100) {
    throw new Error(`Contour is not bound to the Live2D transform: ${JSON.stringify(metrics)}`)
  }
