...
describe('phase 4 PetWindow integration', () => {
  ...
  it('keeps the chat prompt above the fullscreen pet window', () => {
    ...
    expect(manager).toContain("if (kind === 'chat' || kind === 'voice-input') window.setAlwaysOnTop(true, 'screen-saver')")
    expect(manager).toContain("if (kind === 'chat' || kind === 'voice-input') existing.setAlwaysOnTop(true, 'screen-saver')")
    expect(manager).toContain("if (kind === 'chat' || kind === 'voice-input') existing.moveTop()")
  })
  ...
  it('renders music once as an independent stage overlay with selectable geometries', () => {
    ...
    expect(contour).toContain('positionOverlay(overlay, sourceBounds, stageBounds, visualizerStyle)')
