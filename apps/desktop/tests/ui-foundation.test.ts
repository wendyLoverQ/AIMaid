...
describe('renderer UI foundation', () => {
  ...
  it('keeps the responsive system-settings page vertically scrollable', () => {
    ...
    expect(styles).toContain('.settings-workspace { grid-template-columns: 15rem minmax(0, 1fr); width: min(100%, 70rem); margin-inline: auto; }')
    expect(styles).not.toContain('.settings-workspace { grid-template-columns: minmax(0, 1fr); grid-template-rows: auto minmax(0, 1fr); height: 100%; }')
